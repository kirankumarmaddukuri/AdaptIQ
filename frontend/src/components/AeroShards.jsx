...
export default function AeroShards({
  ...
  // Fallback Canvas2D renderer if WebGPU is unavailable or fails
  useEffect(() => {
    ...
    const shards = Array.from({ length: 65 }, (_, i) => ({
      x: Math.random() * width,
      y: Math.random() * height,
      z: 0.3 + Math.random() * 0.7,
      size: (18 + Math.random() * 26) * shardSize,
      rot: Math.random() * Math.PI * 2,
      vRot: (Math.random() - 0.5) * 0.02 * spin,
      speedX: (Math.random() - 0.5) * 0.4 + (flow === 'stream' ? 0.8 : 0.2),
      speedY: (Math.random() - 0.5) * 0.4 + (flow === 'stream' ? 1.2 : 0.3)
    }));
    ...
    const render = () => {
      ...
      const glowGrad = ctx.createRadialGradient(width * 0.5, height * 0.4, 40, width * 0.5, height * 0.4, width * 0.7);
      glowGrad.addColorStop(0, 'rgba(168, 85, 247, 0.12)');
      glowGrad.addColorStop(0.5, 'rgba(137, 106, 189, 0.06)');
      glowGrad.addColorStop(1, 'rgba(18, 15, 23, 0)');
      ctx.fillStyle = glowGrad;
      ctx.fillRect(0, 0, width, height);

      shards.forEach(s => {
        s.x += s.speedX * speed * s.z;
        s.y += s.speedY * speed * s.z;
        s.rot += s.vRot;
        ...
        const pGrad = ctx.createLinearGradient(-s.size, -s.size, s.size, s.size);
        pGrad.addColorStop(0, 'rgba(168, 85, 247, 0.7)');
        pGrad.addColorStop(0.5, 'rgba(137, 106, 189, 0.55)');
        pGrad.addColorStop(1, 'rgba(216, 180, 254, 0.3)');
        ctx.fillStyle = pGrad;
  ...
  }, [useFallback, backgroundColor, shardSize, speed, spin, flow]);
  ...
  useEffect(() => {
    ...
    const reportFailure = error => {
      ...
      // Fallback to high-performance Canvas2D
      setUseFallback(true);
    };
  ...
  return (
    ...
    >
      {useFallback ? (
        <canvas ref={fallbackCanvasRef} className="aero-shards__canvas" style={{ opacity: 1 }} />
      ) : (
